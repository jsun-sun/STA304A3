...
We are interested in explaining whether a person own a house based on age, gender, province of residence, highest education level, family income, and marital status. Let $y_{i} = 1$ if the respondent owns a house (or any kind of dwelling). The model is as the following:
...
model1 <- brm(formula = formula1, data = data,family = bernoulli(link = "logit"), chains = 4, cores = 4, control = list(adapt_delta=.99), seed = 1992)
...
N <- round(nrow(data)/10,0)

# Draw a random sample from the full dataset
data_sub <- data[sample(nrow(data),N),]

# Prediction
pred_model <- t(posterior_predict(model1, newdata = data_sub, nsamples = 500))
...
Pop <- t(posterior_predict(model1, newdata = sample, nsamples = 500))
Pop[is.na(Pop)] <- mean(Pop,na.rm = TRUE)


# Plot    
hist(data_sub$O,breaks = seq(0,50,2), col="grey", prob = TRUE, main='Histogram of Openness Score',xlab="Score")# prob=TRUE for probabilities not counts
    for (i in 1:500){
        lines(density(model1[,i],adjust = 4), col="blue", lwd=0.2) # add a density estimate with defaults
    }
    dev.off()
